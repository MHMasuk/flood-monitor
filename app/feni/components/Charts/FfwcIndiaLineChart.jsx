...
const FfwcIndiaLineChart = ({
    ...
    if (initialLoading) {
        return (
            <div
                className="w-full flex items-center justify-center border border-gray-200 bg-white rounded-lg"
                style={{ height: chartHeight + 'px' }}
            >
                <div className="text-center">
                    <div className="loading loading-spinner loading-md"></div>
                    <p className="text-gray-500 mt-2">{language === 'bn' ? `${stationCode} লোড হচ্ছে...` : `Loading ${stationCode}...`}</p>
                </div>
            </div>
        );
    }
